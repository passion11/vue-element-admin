  ...
  <div class="shop-info">
    <div class="info-content">
      ...
      <section class="section">
        ...
        <div class="activity">
          <div v-for="(support, index) in shopInfo.supports"
               class="activity-item"
               :class=""
               :class="showColor[support.type]"
               :key="index"
          >
      ...
      <section class="section">
        ...
        <div class="pic-wrapper">
          <ul class="pic-list"
              ref="picsUl">
            <li v-for="(pic, index) in shopInfo.pics"
                class="pic-item"
                :key="index">
              <img height="90"
                   width="120"
...
<script>
  ...
  export default {
    data() {
      return {
        showColor: ['activity-red', 'activity-green', 'activity-orange']
      };
    },
    computed: {
      ...mapState(['shopInfo'])
    },
    //因为数据在shop.vue就拿到了,只是缺少组件对象,当前组件对象是在当前路由加载的时候创建的
    //而better-scroll的效果需要数据与组件对象,所以定义在mounted里面创建better-scroll对象
    mounted() {
      this.init;
    },
    updated() {
