<template>
  <div class="footer_guide border-1px">
    <a class="guide_item"
       href="javascript:;"
       :class="{on : '/msite' === $route.path}">
      <span class="item_icon">
        <i class="iconfont iconU"></i>
      </span>
      <router-link to="/msite"><span>外卖</span></router-link>
    </a>
    <a class="guide_item"
       href="javascript:;"
       :class="{on : '/search' === $route.path}">
      <span class="item_icon">
        <i class="iconfont iconsoushuo"></i>
      </span>
      <router-link to="/search"><span>搜索</span></router-link>
    </a>
    <a class="guide_item"
       href="javascript:;"
       :class="{on : '/order' === $route.path}">
      <span class="item_icon">
        <i class="iconfont iconicon-"></i>
      </span>
      <router-link to="/order"><span>订单</span></router-link>
    </a>
    <a class="guide_item"
       href="javascript:;"
       :class="{on : '/profile' === $route.path}">
      <span class="item_icon">
        <i class="iconfont iconicon_user"></i>
      </span>
      <router-link to="/profile"><span>我的</span></router-link>
    </a>
  </div>
</template>
<script>
  export default {};
</script>
<style lang="stylus" rel="stylesheet/stylus">
  @import "../../common/stylus/mixins.styl" // ../在文件夹之间表示向上跳一级
  .footer_guide //footer
    top-border-1px(#e4e4e4)
    position fixed
    z-index 100
    left 0
    right 0
    bottom 0
    background-color #fff
    width 100%
    height 50px
    display flex

    .guide_item
      display flex
      flex 1
      text-align center
      flex-direction column
      align-items center
      margin 5px
      color #999999

      &.on
        color #02a774

      span
        font-size 12px
        margin-top 2px
        margin-bottom 2px

        .iconfont
          font-size 22px
</style>

