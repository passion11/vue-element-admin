  ...
  <div v-if="shopInfo.supports"
    ...
    <nav class="shop-nav"
      ...
      <a class="back"
        ...
        <i class="iconfont iconicon_arrowleft"></i>
      </a>
    </nav>
...
<span class="mini-tag">品牌</span>
        ...
        <div class="shop-message">
          ...
          <span class="shop-message-detail">{{shopInfo.distance}}</span>
        </div>
      </div>
    </div>
    <div class="shop-header-discounts">
      ...
      <div class="discounts-right"
           @click="getSupports">
        {{shopInfo.supports.length-1}}个优惠
      </div>
    </div>
    <transition name="fade">
      <div v-show="showHeadAlert"
  ...
  export default {
    data() {
      return {
        showHeadAlert: false,
        showSupports: false,
        typeTOColor: [{'key': '0', 'val': 'activity-red'},
          {'key': '2', 'val': 'activity-green'},
          {'key': '4', 'val': 'activity-orange'}
        ]
      };
    },
    computed: {
      ...mapState(['shopInfo']),
      getColor(index) {
        const {shopInfo, typeToColor} = this;
        let type = shopInfo.supports[index].type;
        for (let i = 0; i < 3; i++) {
          if (typeToColor[i].key === type) {
            return typeToColor[i].val;
          }
        }
      }
    },
