...
export const reqGoodsList = () => ajax(BASE_URL + '/index_category');
// 3、根据经纬度获取商铺列表](#3根据经纬度获取商铺列表)
export const reqShopsList = (latitude, longitude) => ajax(BASE_URL + '/shops', {latitude, longitude});
// 4、根据经纬度和关键字搜索商铺列表](#4根据经纬度和关键字搜索商铺列表)
export const reqShopListByGeoAndKey = (geohash, keyword) => ajax(BASE_URL + '/search_shops/', {geohash, keyword});
// 5、获取一次性验证码](#5获取一次性验证码)
export const reqCaptcha = () => ajax(BASE_URL + '/captcha');
// 6、用户名密码登陆](#6用户名密码登陆)
export const loginByPwd = ({name, pwd, captcha}) => ajax(BASE_URL + '/login_pwd', {name, pwd, captcha}, 'POST');
// 7、发送短信验证码](#7发送短信验证码)
...
export const userInfo = () => ajax(BASE_URL + '/userinfo');
// 10、用户登出](#10用户登出)
